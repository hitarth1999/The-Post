        ...
        <div class="mt-8 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg">
            <div class="grid grid-cols-1">
                <div class="p-6 border-t border-gray-200 border-t-0 border-l">
                    <div class="grid grid-cols-1">
                        <form action="{{route('post.update',['post' => encrypt($post->id)])}}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="title">Title *</label>
                                <input class="form-control" type="text" name="title" id="title" required autofocus value="{{old('title', $post->title)}}">
                            </div>
                            <div class="form-group">
                                <label for="description">Description *</label>
                                <textarea class="form-control" name="description" required id="description">{{old('description', $post->description)}}</textarea>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="date" name="post_date" required id="post_date" value={{old('post_date', $post->post_date)}} max="{{today()}}">
                            </div>
